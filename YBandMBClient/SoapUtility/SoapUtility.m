...
-(NSString *)BuildSoapwithMethodName:(NSString *)methodName
                        WithWSDLName:(NSString *)wsdlName
                           withParas:(NSDictionary *)parasdic
                      withtargetName:(NSString *)targetName
                   withNamespaceName:(NSString *)spaceName
{
    //根节点
    DDXMLElement *ddRoot = [DDXMLElement elementWithName:@"soapenv:Envelope"];
    //根节点的命名空间
    [ddRoot addNamespace:[DDXMLNode namespaceWithName:@"soapenv" stringValue:@"http://schemas.xmlsoap.org/soap/envelope/"]];
    [ddRoot addNamespace:[DDXMLNode namespaceWithName:spaceName stringValue:targetName]];
    //头
...
//    [msg addNamespace:ddmsgNS];
    ...
    DDXMLElement *msg=[DDXMLElement elementWithName:[NSString stringWithFormat:@"%@:%@",wsdlName,methodName]];
